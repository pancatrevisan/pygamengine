...
class CollisionBox:
    ...
    def isPointInside(self, p):
        tanslated = self
        #rec = [translated.rect[0], translated.rect[1], translated.rect[0]+translated.width, translated.rect[1]+]
        print("check " + str(p) + " rc: " + str(tanslated.rect))

        if p[0] >= tanslated.rect[0] and p[0] <= (tanslated.rect[2]):
            if p[1]>=tanslated.rect[1] and p[1] <= (tanslated.rect[3]):
                return True
        return False
    ...
    def computeOnScreen(self):
        ...
        w =[0,0,800,600] #mudar pela do mapa.
        tBox = self.computeTranslated().rect
        box_h = tBox[3] - tBox[1]
        box_w = tBox[2] - tBox[0]
        #print("size: " + str([box_w, box_h]))
        #print("Tbox: " + str(tBox))
    ...
    def computeTranslated(self):
        ...
        #print("spr on worl: " +str(spriteOnWorld))
